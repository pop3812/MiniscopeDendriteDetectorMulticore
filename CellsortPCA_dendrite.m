function [mixedsig, mixedfilters, CovEvals, covtrace, movm, ...
    movtm] = CellsortPCA_dendrite(ms, inputPath, flims, nPCs, dsamp, outputdir, badframes)
...
% Determine how many frames there are. % Kwang
...
% [~, fname] = fileparts(params.inputPath);
fname = ms.vidObj{1}.Name;
...
dsamp = [1,space_down_samp_rate];
pixw = ceil(ms.alignedHeight./space_down_samp_rate);
pixh = ceil(ms.alignedWidth./space_down_samp_rate);
npix = pixw*pixh;
...
if nt < npix 
    ...
    [covmat, mov, movm, movtm] = create_tcov(ms, pixw, pixh, useframes, nt, dsamp);
else
    fprintf(' using spatial covariance matrix.\n')
    [covmat, mov, movm, movtm] = create_xcov(ms, pixw, pixh, useframes, nt, dsamp);
end

covtrace = trace(covmat) / npix;
movm = reshape(movm, pixw, pixh); % fixed
...
if nPCs > size(mixedfilters,2)
    nPCs=size(mixedfilters,2);
end
...
toc
    ...
    function [covmat, mov, movm, movtm] = create_xcov(ms, pixw, pixh, useframes, nt, dsamp)
        %-----------------------
    ...
    function [covmat, mov, movm, movtm] = create_tcov(ms, pixw, pixh, useframes, nt, dsamp)
        %-----------------------
