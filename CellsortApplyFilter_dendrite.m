function cell_sig = CellsortApplyFilter_dendrite(ms, ica_segments, flims, movm, subtractmean)
...
pixw = ceil(ms.alignedHeight./ms.space_down_samp_rate);
pixh = ceil(ms.alignedWidth./ms.space_down_samp_rate);
...
while k<nt
    ...
    parfor j=1:ntcurr %%%
        movcurr = imresize(msReadFrame(ms,j+k+flims(1)-1, true, true, false), 1/ms.space_down_samp_rate, 'bilinear')';
        mov(:,:,j) = movcurr';
    end
    
    % using min as baseline
    movm = imresize(ms.minFrame{ms.selectedAlignment}, 1/ms.space_down_samp_rate, 'bilinear');
    mov = (mov ./ repmat(movm, [1,1,ntcurr])) - 1;
